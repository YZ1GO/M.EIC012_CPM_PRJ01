...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!groupId || !expenseId || !imageBase64) {
      return new Response(JSON.stringify({ error: "Missing fields: groupId, expenseId, imageBase64" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const bucket = "cleave_receipts";
    const objectPath = `groups/${groupId}/expenses/${expenseId}/receipt.jpg`;
    ...
    // Keep URL valid for a long time so existing mobile code can store/use it directly.
    ...
    return new Response(JSON.stringify({ receiptUrl: data.signedUrl }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
